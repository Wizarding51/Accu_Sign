...
const SideNavSampleSideNav = ({ className = "" }) => {
  const onSideNavItemsClick = useCallback(() => {
    // Please sync "Trip" to the project
  }, []);
  ...
  const onSideNavItemsClick2 = useCallback(() => {
    // Please sync "Document Uploaded" to the project
  }, []);

  const onSideNavItemsClick3 = useCallback(() => {
    // Please sync "Aircraft" to the project
  }, []);

  return (
    <div
      className={`h-[840px] w-[260px] flex flex-col items-start justify-start relative text-left text-sm text-active font-body mq1125:hidden ${className}`}
    >
      ...
      <img
        ...
        src="/settings.svg"
      />
      <div className="w-full h-[49px] !m-[0] absolute top-[0px] right-[0px] left-[0px] flex flex-col items-start justify-start pt-3 px-0 pb-[13px] box-border gap-[13px] z-[2] text-white">
        <img
